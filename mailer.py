...
def send_reset_mail(email, token):
    ...
    email = {
        ...
        'html': f'<p>Для восстановления пароля пройдите по ссылке: http://www.mp-scope.tech/forgot?token={token}</p>',
        'text': f'Для восстановления пароля пройдите по ссылке: http://www.mp-scope.tech/forgot?token={token}',
        'from': {'name': 'Команда MP-Scope', 'email': 'service@mp-scope.tech'},
        'to': [
            {'name': f'"{email}"', 'email': email}
        ]
